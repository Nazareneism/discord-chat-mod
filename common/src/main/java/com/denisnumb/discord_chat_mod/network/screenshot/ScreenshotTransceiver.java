        ...
        networkPool.execute(() -> {
            try {
                ...
                BigPacketsTransceiver.send(data, (partIndex, totalParts, part) ->
                        PlatformPacketDistributor.sendToServer(new ScreenshotPartPacket(currentTime, partIndex, totalParts, part))
                );
            } catch (Exception e) {
        ...
        networkPool.execute(() -> {
            try {
                ...
                for (ServerPlayer player : playerList) {
                    try {
                        BigPacketsTransceiver.send(data, (partIndex, totalParts, part) ->
                                PlatformPacketDistributor.sendToPlayer(player, new ScreenshotPartPacket(currentTime, partIndex, totalParts, part))
                        );
